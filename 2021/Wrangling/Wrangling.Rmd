...
- does not change input types (not up to date)
...
<ol start="1"> 
<li>  Load the `nycflights13::flights` data set. Does it come formatted as a tibble? </li>
...
<li>  Brainstorm as many ways as possible to select `dep\text{_}time`, `dep\text{_}d elay`, `arr\text{_}time`, and `arr\text{_}d elay`. </li>
<li>  Create a new variable `first\text{_}flight` which takes the value `TRUE` if it is the first flight of the day. </li>
</ol>
...
<li>  What was the most dangerous hour of the week according to the number of homicides (`OFFENSE\text{_}CODE\text{_}GROUP == "Homicide"`)?  </li>
<li>  Calculate for each district the proportion of motor vehicle accidents to all reported incidents (`OFFENSE\text{_}CODE\text{_}GROUP == "Motor Vehicle Accident Response"`).   </li>
...
shooting was involved for each district. Now take a random sample of size 10.000. Is there a noteworthy difference? **Hint:** There is a variable called `SHOOTING`.
  </li>
</ol>
